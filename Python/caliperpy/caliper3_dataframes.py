...
def read_dtypes(fname, debug_msgs=False):
    ...
    with open(dcb_fname) as dcb_file:
        ...
        for aLine in dcb_file:
            ...
            if len(aLine) > 0 and len(aLine) <= 2:
                coding = 'cp1252'
                try:
                    #aLine = aLine[0].split()
                    ...
                    elif 'codepage' in aLine[1]:
                        coding = f"cp{aLine[1].split('=')[1]}"
                except:
                    pass
    ...
    return dt_list, tcType_list, row_byte_len, coding
...
def write_dcb_file(outFilename, df, col_dtypes, dt_cols, nBytesPerRow, coding='cp1252'):
    '''
    ...
    outcoding = f"codepage={coding.split('cp')[1]}"
    outRows = [[], [str(nBytesPerRow), outcoding]]
...
def set_na_str_values(df, coding='cp1252'):
    '''
