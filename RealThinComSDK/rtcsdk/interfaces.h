...
#pragma region object
		...
		inline void *query(T *pobj, const GUID &iid, impl::vector<Interfaces...>) noexcept
		{
			void *result{ nullptr };
		...
		struct __declspec(empty_bases)extends_base : public Interfaces...
		{
			...
			{
				return query(pobject, riid, impl::vector<Interfaces... > {});
			}
		};
		...
		struct __declspec(empty_bases)extends : public extends_base<Interfaces...>
		{
			struct can_query
			{
				using type = impl::vector<Interfaces...>;
			};
		...
		struct __declspec(empty_bases)intermediate : public extends_base<FirstInterface, RestInterfaces...>
		{
			struct can_query
			{
				using type = impl::vector<FirstInterface, RestInterfaces...>;
			};
		...
		struct __declspec(empty_bases)also	// no inheriting from interfaces!
		{
			...
			static void *query_self(Derived *pobject, const GUID &iid) noexcept
			{
				return query(pobject, iid, impl::vector<Interfaces... > {});
			}
		};
		...
		//template<class T, class...Args>
		//struct has_final_construct_impl<T, impl::vector<Args...>, decltype(void(std::declval<T &>().FinalConstruct(std::declval<Args>()...)))> : std::true_type {};

		//template<class T, class...Args>
		//using has_final_construct = has_final_construct_impl<T, impl::vector<Args...>, void>;
		...
		constexpr bool check_trait_vector(impl::vector<Interfaces...>) noexcept
		{
...
#define RTCSDK_CLASS_GUID(id) static constexpr auto get_guid() noexcept { constexpr auto guid = rtcsdk::make_guid(id); return guid; }
#define RTCSDK_CLASS_GUID_EXISTING(id) static constexpr auto get_guid() noexcept { return id; }
...
struct __declspec(novtable) __declspec(empty_bases) __declspec(uuid(id)) name : rtcsdk::extends<name, IUnknown> \
// end of macro

#define RTCSDK_DEFINE_INTERFACE_BASE(name,base,id) \
_RTCSDK_GUID_HELPER(name, id) \
struct __declspec(novtable) __declspec(empty_bases) __declspec(uuid(id)) name : rtcsdk::extends<name, base> \
// end of macro
